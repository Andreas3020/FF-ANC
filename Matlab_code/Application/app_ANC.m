classdef app_ANC < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        NumberoffilterweightsLabel      matlab.ui.control.Label
        ...
        fp1_3                           matlab.ui.control.UIAxes
        ...
        fp1_2                           matlab.ui.control.UIAxes
    end
...
%--------------------------------------------------------------------------------------------------------------------            
            ...
            elseif domain == "Time-frequency domain" 
                ...
                %error_mic = lowpass(error_mic, 1000, fs_pn);
...
%--------------------------------------------------------------------------------------------------------------------              
            ...
            %mean_difference = mean(SNR_after(end-50:end) - SNR_before(end-50:end));   
            %app.DifferenceinSNRTextArea.Value = num2str(mean_difference);
            
            pks_before = findpeaks(SNR_before);
            pks_after = findpeaks(SNR_after);
            
            if length(pks_after) >= 7
                for i = 1 : 7
                   difference(i) = pks_after(end+1-i) - pks_before(end+1-i);
                end
...
%--------------------------------------------------------------------------------------------------------------------              
            ...
            error_only = filter(lpB1000,error_only);
            speech = filter(lpB1000,speech);
...
%--------------------------------------------------------------------------------------------------------------------
        ...
        function sounderror2ButtonPushed(app, event)
            ...
            sound(error_microphone_signal*10,fs_pn);
        end
...
%-------------------------------------------------------------------------------------------------------------------- 
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create NumberoffilterweightsLabel
            app.NumberoffilterweightsLabel = uilabel(app.ParameterControlPanel);
            app.NumberoffilterweightsLabel.Position = [24 132 132 22];
            app.NumberoffilterweightsLabel.Text = 'Number of filter weights';
            ...
            app.p1.PlotBoxAspectRatio = [1 0.670391061452514 0.670391061452514];
            ...
            app.sp1.PlotBoxAspectRatio = [1 0.689655172413793 0.689655172413793];
            app.sp1.YLim = [0 1000];
            ...
            ylabel(app.ap1, '|P(f)|')
            app.ap1.PlotBoxAspectRatio = [1 0.679886685552408 0.679886685552408];
            app.ap1.XLim = [0 1000];
            ...
            % Create fp1_3
            app.fp1_3 = uiaxes(app.FrequencyresponsTab_3);
            title(app.fp1_3, 'Frequency response of residual error after ANC')
            xlabel(app.fp1_3, 'frequency (Hz)')
            ylabel(app.fp1_3, 'Power (dB)')
            app.fp1_3.BackgroundColor = [1 0.9882 0.9882];
            app.fp1_3.Position = [19 15 405 295];
            ...
            % Create fp1_2
            app.fp1_2 = uiaxes(app.FrequencyresponsTab_4);
            title(app.fp1_2, 'Frequency response of loudspeaker')
            xlabel(app.fp1_2, 'frequency (Hz)')
            ylabel(app.fp1_2, 'Power (dB)')
            app.fp1_2.BackgroundColor = [1 0.9882 0.9882];
            app.fp1_2.Position = [18 18 405 295];
        end
    end
