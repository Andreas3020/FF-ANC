...
void* calc(void* cArgs){
  ...
  calcArgs args = *(calcArgs*)cArgs;
    ...
    for(int i = 0; i < 4; i++){
      coefs[i] = (fftw_complex*) malloc(USED_BINS * sizeof(fftw_complex));
    ...
    for(int i = 0; i < 4; i++){
      p1[i] = (fftw_complex*) malloc(USED_BINS * sizeof(fftw_complex));
      p2[i] = (fftw_complex*) malloc(USED_BINS * sizeof(fftw_complex));
      memset( p1[i], 0, USED_BINS * sizeof(fftw_complex));
      memset( p2[i], 0, USED_BINS * sizeof(fftw_complex));
    }
...
void transformation_func(int* resultPos, npy_float64 input[LENGTH + interpropellerDelay][5], fftw_complex** coefs, npy_float64 output[LENGTH], npy_float64 prevOutput[LENGTH], fftw_complex* prevError, fftw_complex** p, int N_STFT, int R_STFT, int FFT_HALF, npy_float64* FFTin, fftw_complex* FFTout, fftw_complex* IFFTin, npy_float64* IFFTout, fftw_plan planFFT, fftw_plan planIFFT){
  // init the variables
  npy_float64* norm = fftw_malloc(USED_BINS * sizeof(npy_float64));
