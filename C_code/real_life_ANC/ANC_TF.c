/******************************* RBFm.C ***********************************
  ...
  *
  *
  * Programs purpose is to use Active Noise Cancellation  in the time-frequency
  * domain to cancel out the propellor noise of the UAV. The program uses 3
  * threads and 1 main thread.
  *
...
int main(){
  ...
  calcArgs* cArgs = &(calcArgs){.duration = duration, .stepsize = stepsize, .mut1r = mutex1r, .mut2r = mutex2r, .mut1p = mutex1p, .mut2p = mutex2p, .cond11 = cond11, .cond21 = cond21, .cond12 = cond12, .cond22 = cond22, .cond13 = cond13, .cond23 = cond23, .written1 = written1, .written2 = written2, .N_STFT = N_STFT, .R_STFT = R_STFT};
